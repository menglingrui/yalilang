 ...
 * 
 * 表体自定义项  名称改变 使表头和表体名字不一样
 * 
 * @author zhf  edit mlr
 * 
 */
public abstract class DefBillManageUI1 extends BillManageUI implements ILinkQuery{
	...
	protected BillData changeBillDataByUserDef(DefVO[] defHead,
		...
		try {
			...
			// 表体
			if ((defBody != null)) {
				oldBillData.updateItemByDef(defBody, "vdefb", false);
				for (int i = 1; i <= 20; i++) {
					nc.ui.pub.bill.BillItem item = oldBillData
							.getBodyItem("vdefb" + i);
					if (item != null) {
	...
	protected void afterVuserDefEdit(BillEditEvent e) {
		...
		if (pos == 0) {// 表头
			...
			String sVdefPkKey = "pk_defdocb"
					+ sItemKey.substring("vdefb".length());
