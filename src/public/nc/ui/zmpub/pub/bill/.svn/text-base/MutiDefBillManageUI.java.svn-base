 ...
 *
 */
public abstract class MutiDefBillManageUI extends MultiChildBillManageUI implements ILinkQuery{
	...
	protected BillCardPanelWrapper createBillCardPanelWrapper()
			throws Exception {
		ZMMultiChildBillCardPanelWrapper card=new ZMMultiChildBillCardPanelWrapper(
...
//				}
	...
	protected void initSelfData() {// 表体行显示零  
		...
		ButtonObject btnobj1 = getButtonManager().getButton(IBillButton.Delete);
		if (btnobj1 != null) {
			if(btnobj1.getData()!=null){
				nc.vo.trade.button.ButtonVO btn=(ButtonVO) btnobj1.getData();
				btn.setBusinessStatus(new int[]{IBillStatus.FREE});
			}
		}
