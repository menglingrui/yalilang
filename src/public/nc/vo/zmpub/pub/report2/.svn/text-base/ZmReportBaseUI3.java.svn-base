...
public class ZmReportBaseUI3 extends ZmReportBaseUI2{
	...
	public ReportBuffer queryBuffer(String modelCode) {
		...
		try {
			...
			Object o = LongTimeTask.calllongTimeService(ZmPubConst.module, this,
  ...
  protected String getQueryConditon()
	throws Exception {
		...
		ConditionVO[] vos2=filterBufferQuery(vos1);
		String sql=querylg.getWhereSQL(vos2);
		return sql;
  }
  public ConditionVO[] filterBufferQuery(ConditionVO[] vos) {
		   List<ConditionVO> list=new ArrayList<ConditionVO>();
		   if(vos==null || vos.length==0)
			   return null;
		   for(int i=0;i<vos.length;i++){		   
		      if(vos[i].getFieldCode().startsWith("sc")){
		    	  continue;
		      }
		      list.add(vos[i]);
		   }
