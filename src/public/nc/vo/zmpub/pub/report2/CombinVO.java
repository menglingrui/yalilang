...
/**
 * 主要用于报表的处理，支持报表数据的过滤，合并，追加等业务处理
 * 也可以用于业务单据
 * @author mlr
 *
 */
public class CombinVO {
	...
	/**
	 * 完达山物流项目 根据某个维度(条件) 将两个数组中条件字段对应值相同的合并 根据 求值字段数组和类型数组 判断需要求和的字段 进行求和运算
	 * 
	 ...
	 * @throws Exception
	 */
	public static CircularlyAccessibleValueObject[] comin(CircularlyAccessibleValueObject[] vos,CircularlyAccessibleValueObject[] vos1)throws Exception{
		if (isEmpty(vos)) {
			if (!isEmpty(vos1)) {
				return vos1;
			}
		...
		return list.toArray((CircularlyAccessibleValueObject[])java.lang.reflect.Array.newInstance(vos[0].getClass(), 0));
	}
	/**
 ...
 public static ReportBaseVO[] addByContion2(ReportBaseVO[] vos,ReportBaseVO[] vos1,String[] voCombinConds,String xid)throws Exception{
		 ...
		 * 
		 * @author mlr  
		 * @param num_condition_fields 合并分组维度
		 * @param combinFields 合并求和字段
		 * @return
		 * @throws Exception 
		 */
	...
	public static CircularlyAccessibleValueObject[] combinData(
		...
		if(dataMap.size() == 0)
			return null;
		CircularlyAccessibleValueObject[] tarVos = (CircularlyAccessibleValueObject[])java.lang.reflect.Array.newInstance(vos[0].getClass(), dataMap.size());
	    dataMap.values().toArray(tarVos);
	    return tarVos;
	}
