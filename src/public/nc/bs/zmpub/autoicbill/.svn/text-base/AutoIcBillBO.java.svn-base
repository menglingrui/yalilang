...
//		自动拣货
	...
	throws BusinessException{
		...
		if(items==null || items.length==0){
			throw new BusinessException("表体数量为空");
		}
	...
	public void dealOnUnApprove(
			...
			boolean isOut,
			String coutbilltype,
			boolean isInCancelSign,
			boolean isOutCancelSign
			) 
	throws BusinessException{
		if(bill == null)
			return;
		...
		if(!isInCancelSign)
			checkExitNextBills(headid,ScmConst.m_otherIn,false);
		if(!isOutCancelSign)
			checkExitNextBills(headid,ScmConst.m_otherOut,false);
		//		自动删除 出库单 
		if(isIn)
			deleteNextBillBySourceID(bill.getParentVO().getPrimaryKey(), cinbilltype, date,cuser,isInCancelSign);
		if(isOut)
			deleteNextBillBySourceID(bill.getParentVO().getPrimaryKey(), coutbilltype, date,cuser,isOutCancelSign);
	}	
	/**
	 * 
	 * @author zhf
	...
	public void dealOnUnApprove2(
			...
			String coutbilltype,
			boolean isInCancelSign,
			boolean isOutCancelSign) throws BusinessException{
		if(bill == null)
			return;
		//		检验是否下游单据已签字  出库单+入库单
		String headid = bill.getParentVO().getPrimaryKey();
		if(PuPubVO.getString_TrimZeroLenAsNull(headid)==null)
			throw new BusinessException("数据异常");
		if(!isInCancelSign)
			checkExitNextBills(headid,ScmConst.m_otherIn,true);
		if(!isOutCancelSign)
			checkExitNextBills(headid,ScmConst.m_otherOut,true);
		//		自动删除 出库单  入库单
		if(isIn)
			deleteNextBillByFirstID(bill.getParentVO().getPrimaryKey(), ScmConst.m_otherIn, date,cuser,isInCancelSign);
		if(isOut)
			deleteNextBillBySourceID(bill.getParentVO().getPrimaryKey(), ScmConst.m_otherOut, date,cuser,isOutCancelSign);
	}
