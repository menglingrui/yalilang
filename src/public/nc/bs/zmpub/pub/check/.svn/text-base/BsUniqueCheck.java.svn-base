...
public class BsUniqueCheck {
	   ...
	   private static void queryByCheckFields(SuperVO headVo,SuperVO bodyVo,String[] checkFields,String headconditon,String bodyconditon,String errorMessage) throws Exception{
			  ...
			  if(isEmpty(bodyVo))
		       ...
		       StringBuffer cond = new StringBuffer();
				...
				String condition = cond.toString().substring(0,length-4) + " and  isNull("+ bodyVo.getTableName() + ".dr,0)=0";
				List list = (List) getDao().retrieveByClause(bodyVo.getClass(),condition);
				String sql=" select count(*) from "+bodyVo.getTableName()+" join "+headVo.getTableName()+" on "+bodyVo.getTableName()+"."+bodyVo.getParentPKFieldName()+"="+
				headVo.getTableName()+"."+headVo.getPKFieldName()+" where "+condition+headconditon+bodyconditon;
	...
	private static void queryByCheckField(SuperVO vo, String checkField,
		 ...
		 String sign=" = ";
			...
			String condition = checkField +sign + value+ " and  isNull("+vo.getTableName()+".dr,0)=0 "+condition1;
	 ...
	 * 组合字段的区间交叉的唯一性校验
	 *       即校验某个维度下，在最小值和最大值之间只存在 一条记录
	 *       用于单据的新增保存和修改保存时唯一校验字段(checkFields)已经被修改 的时候
	 * @作者：mlr
	 * @说明：完达山物流项目 
	...
	private static void queryByCheckField(SuperVO vo,String checkField,String errorMessage) throws Exception{
		 String sign=" = ";
			...
			String condition = checkField +sign + value+ " and  isNull("+vo.getTableName()+".dr,0)=0";
			List list = (List) getDao().retrieveByClause(vo.getClass(),
					condition);
			if (list == null || list.size() == 0) {
				return;				
	 ...
	 */
	 private static List queryByPrimaryKey(SuperVO vo) throws Exception{
			...
			+ "' and  isNull("+vo.getTableName()+".dr,0)=0";
	...
	/**
	   ...
	   private static void queryByCheckFields(SuperVO vo, String[] checkFields,
		   ...
		   StringBuffer cond = new StringBuffer();
			...
			String condition = cond.toString().substring(0,length-4) + " and  isNull("+ vo.getTableName() + ".dr,0)=0 " +conditon;
			List list = (List) getDao().retrieveByClause(vo.getClass(),condition);
			if (list == null || list.size() == 0) {
				return;
			} else {
   ...
   private static void queryByCheckFields(SuperVO vo,String[] checkFields,String errorMessage) throws Exception{
			...
			int length=cond.toString().length();
			String condition = cond.toString().substring(0,length-4) + " and  isNull("+ vo.getTableName() + ".dr,0)=0";
			List list = (List) getDao().retrieveByClause(vo.getClass(),condition);
			if (list == null || list.size() == 0) {
				return;
			} else {
    ...
    */
	public static boolean isChar(Object value) {
		...
		if(value instanceof String || value instanceof UFDate || value instanceof UFBoolean){
		 return true;	
		}
		return false;
	}
