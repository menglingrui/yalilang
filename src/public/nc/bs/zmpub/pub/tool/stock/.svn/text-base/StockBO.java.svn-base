...
/**
 * 定义台账 库存现存量的公共处理方法什么是现存量？现存量就是库存状态的变化的数据的展现,做入库单现存量肯定要增,做出库单现存量肯定要减
 * 
 * 用来用记录这个当前库存量的表就叫做现存量表
 * 
 * 本工具类是用来操作现存量变化的：什么时候用呢？当你做到单据要影响到现存量的时候就可以用本工具类来更新现存量
 * 
 * 也可以用本工具类来按最小维度查询现存量
 * 
...
public abstract class StockBO implements Serializable {
	 ...
	 * @return
	 */
	public abstract String[] getDef_FieldsName();
	/**
     ...
     */
	public void lock(SuperVO[] accounts) throws BusinessException {
		if(accounts==null || accounts.length==0){
			return;
		}
     ...
     * @return
     */
	public String getNumKey(SuperVO inv) {
		if(inv==null){
			return null;
		}
		String key="";
		for(String name:getDef_Fields()){
