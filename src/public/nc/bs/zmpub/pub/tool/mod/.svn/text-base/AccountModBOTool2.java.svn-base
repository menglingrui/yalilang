...
//    }
   ...
   public SuperVO[] filterNUll(SuperVO[] vos) throws Exception {
	...
	private String getClearSql(String whereSql) throws Exception {
		AccountData ad=getAccountData();
		if(ad==null){
		    throw new BusinessException("没有注册现存量信息类");
		}
		if(ad.getClearSql(whereSql)==null ||ad.getClearSql(whereSql).length()==0){
			throw new BusinessException("现存量信息类没有注册清空现存量方法");
		}
		return ad.getClearSql(whereSql);
	}
	/**
