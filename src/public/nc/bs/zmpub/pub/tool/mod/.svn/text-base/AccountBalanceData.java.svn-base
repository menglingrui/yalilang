...
//	   public abstract boolean isloadPeridData(String pk_corp)throws BusinessException;
	   ...
	   * 在什么情况下加载期初数据呢？
<<<<<<< .mine
	   * 月份结账时如果一月份已经结 一月份的结账表中肯定已经加载了期初数据  
	   * 如果一月份已经结账的情况下进行台账修复就不会加载期初数据了
	   * 因为结账时:是从结存表中取已结账的数据转化成现存量,加上最后一个结账月往后的从业务单据上汇总来得数据
	   * 如果一月份未结账那么期初数据在修复的时候,就加载不到现存量中所以必须手动加载
=======
	   * 
